...
def plot_data(distances, times, normalize=False):
    ...
    # Check if the data is normalised to set appropriate labels and title
    if normalize: 
...
def plot_training_progress(epoch, loss, model, distances_norm, times_norm):
    """
    Plots the training progress of a model on normalised data, 
    ...
    # Clear the previous slot from the output cell
    ...
    # Detach predictions from the computation grapp and convert to NumPy
    y_pred_lot = predicted_norm.detach().numpy()  
    ...
    plt.plot(x_plot[sorted_indices], y_pred_lot[sorted_indices], color = 'green', label = 'Model Predictions')

    # Set the title for the plot, including the current epoch
...
def plot_final_fit(model, distances, times, distances_norm, times_std, times_mean):
    """
    Plots the predictions of a trained model againts the original data, 
    after de-normalising the predictions. 
    ...
    with torch.no_grad():
        # Get the predictions from the model using normalized data
        predicted_norm = model(distances_norm)
    
    # De-normalized the predictions to the original scale
    predicted_times = (predicted_norm * times_std) + times_mean

    # Create new fiture for the plot
    plt.figure(figsize=(8,6))
    ...
    plt.plot(distances.numpy(), predicted_times.numpy(), color = 'green', label='Non-Linear Predictions')
